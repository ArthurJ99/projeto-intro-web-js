...
const buscarTurma = (nomeTurma) => {
  ...
  let busca = turmas.filter((obj) =>
    obj.turma.toLowerCase().startsWith(nomeTurma.toLowerCase())
  );
  ...
  } else if (busca.length < 1 && input.value) {
  ...
  const cards = turmasBuscadas.map((turma) => {
    ...
    <article>
        <ul>
            ...
            <li><strong>Concluido: </strong>${
              turma.concluida ? "Sim" : "Não"
            }</li>
        </ul>
    </article>`;
  });
...
const buscarCurso = (nomeCurso) => {
  const busca = cursos.find((element) =>
    element.curso.toLowerCase().startsWith(nomeCurso.toLowerCase())
  );
  return busca;
};
...
const buscarEstudante = (nomeEstudante) => {
  ...
  let buscaAluno = estudantes.filter((obj) =>
    obj.estudante.toLowerCase().startsWith(nomeEstudante.toLowerCase())
  );
  if (!nomeEstudante) {
...
const parcelarCurso = (valorCursos, nParcelas) => {
  ...
  switch (valorCursos.length) {
    ...
    case 1:
      ...
      if (desconto == 1) {
        return `O valor do pagamento é de R$${totalCompra.toFixed(
          2
        )}, parcelado em ${nParcelas}X de R$ ${(
          totalCompra / nParcelas
        ).toFixed(2)}`;
      } else {
        return `O valor do pagamento é de R$${totalCompra.toFixed(2)} com ${(
          100 -
          desconto * 100
        ).toFixed(0)}% de desconto, parcelado em ${nParcelas}X de R$ ${(
          totalCompra / nParcelas
        ).toFixed(2)}`;
      }
    case 2:
...
simuladorButton.addEventListener("click", () => {
  ...
  simuladorResult.innerText = parcelarCurso(
    carrinhoCursos,
    parcelasCurso.value
  );
  parcelasCurso.value = "";
